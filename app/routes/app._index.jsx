        ...
        if (test.status === 'completed' && test.winner) {
          ...
          if (analysis.decision !== 'no_clear_winner') {
            ...
            if (winner === 'B') {
              ...
              // Control (A) won - clear the metafield since no widget in control template
              try {
                let productGid = test.productId;
                if (!productGid.startsWith('gid://')) {
                  const numericId = productGid.match(/Product\/(\d+)/)?.[1] || productGid.match(/^(\d+)$/)?.[1] || productGid;
...
export default function Dashboard() {
  ...
  const [removedIdeas, setRemovedIdeas] = useState(new Set());
  ...
  return (
    ...
    <div style={{ 
      ...
      >
        ...
        `}</style>
        {abTestIdeas.filter(widget => !removedIdeas.has(widget.id)).map((widget, index) => (
          <div
            key={widget.id}
            style={{
              flexShrink: 0,
        ...
        >
          ...
          >
            ...
            <p style={{
              ...
              Are you sure you want to remove this idea from the list?
            </p>
            <div style={{
              display: 'flex',
              gap: '12px',
              ...
              <button
                onClick={() => {
                  if (ideaToRemove) {
                    setRemovedIdeas(prev => new Set([...prev, ideaToRemove.id]));
                  }
                  setShowRemoveModal(false);
                  setIdeaToRemove(null);
                }}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div 
              ...
              onMouseEnter={() => setShowIncrementalATCTooltip(true)}
              onMouseLeave={() => setShowIncrementalATCTooltip(false)}
            >
              ...
              {showIncrementalATCTooltip && (
                <div style={{
                  ...
                  Estimated additional add-to-cart actions generated by winning product page tests.
                </div>
              )}
            </div>
          ...
          <div style={{
            ...
            <div 
              ...
              onMouseEnter={() => setShowWinnersDeclaredTooltip(true)}
              onMouseLeave={() => setShowWinnersDeclaredTooltip(false)}
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ color: '#6B7280' }}>
                <circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.5"/>
                <text x="8" y="11" textAnchor="middle" fontSize="10" fill="currentColor" fontWeight="bold">?</text>
              </svg>
              {showWinnersDeclaredTooltip && (
                <div style={{
                  position: 'absolute',
                  bottom: '100%',
                  right: 0,
                  ...
                  Total count of A/B tests with a declared winner. The monthly change reflects testing momentum.
                </div>
              )}
            </div>
            <div style={{ marginBottom: '20px' }}>
              ...
              <div>
                <p style={{
                  ...
                  {winnersDeclaredChange !== undefined && winnersDeclaredChange !== null
                    ? `${winnersDeclaredChange >= 0 ? '+' : ''}${winnersDeclaredChange.toFixed(1)}%`
                    : 'N/A'
                  }
                </p>
                <p style={{
